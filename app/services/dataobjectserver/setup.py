...
def setup_base_tables(root,db_sql):
    for table in root.findall('./tables/table'):
        ...
        statment = 'create table if not exists {} ('.format(table_name)
        if table.get('id_type') != None:
            statment += 'id {} unsigned NOT NULL AUTO_INCREMENT,'.format(table.get('id_type'))
        for field in table.findall('./fields/field'):
            ...
            statment += ' {0} {1} {2},'.format(field_name,field_type,field_size,)
        statment += ' primary key(id)) ENGINE=InnoDB;'
        db_sql.send(statment)
        print(statment)
...
def setup_related_tables(root,db_sql):
    for table in root.findall('./relatedtables/relation'):
        for child in table.findall('./children/table'):
            ...
            statment = 'create table if not exists {}_map_{} ('.format(table.get('parent'),child.text)
            statment += 'id {} unsigned NOT NULL AUTO_INCREMENT,'.format(related_id_type)
            statment += '{}id {} unsigned NOT NULL,'.format(table.get('parent'),parent_id_type)
            statment += '{}id {} unsigned NOT NULL,'.format(child.text,child_id_type)
            statment += 'FOREIGN KEY (`{0}id`) REFERENCES `{0}` (`id`),'.format(table.get('parent'))
            statment += 'FOREIGN KEY (`{0}id`) REFERENCES `{0}` (`id`),'.format(child.text)
            statment += ' primary key (id)) ENGINE=InnoDB;'
            db_sql.send(statment)
